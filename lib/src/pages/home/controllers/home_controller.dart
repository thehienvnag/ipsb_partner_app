...
List<Choice> getChoice() {
  ...
  if (states.account!.role == 'Store Owner') {
    result.add(Choice(title: 'Scan QR',
      icon: Icons.qr_code,
      color: Color(0xFF4940e2),
      route: Routes.checkQRCode,),);
  }
  if (states.account!.role == 'Store Owner') {
    result.add(Choice(title: 'Coupons', icon: Icons.local_activity,color: Color(0xFFf98127),route: Routes.manageCoupon));}
  if (states.account!.role == 'Store Owner') {
    result.add(Choice(title: 'Notification', icon: Icons.notifications,color: Color(0xFF38ba78),route: Routes.notifications));}
  if (states.account!.role == 'Building Manager') {
    result.add(Choice(title: 'iBeacon', icon: Icons.view_in_ar,color: Color(0xFF38ba78),route: Routes.locatorTag,));}
  result.add(Choice(title: 'Profile', icon: Icons.person,color: Color(0xFFf53147),route: Routes.profile,),);
  return result;
}
